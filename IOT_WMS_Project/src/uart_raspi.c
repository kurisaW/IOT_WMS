...
static void uart_raspi_thread_entry(void *parameter)
{
    ...
    rt_kprintf("3. Upload sensor data   :Please input (3)\n");
    rt_kprintf("4. exit menu        :please input <c> || <C>\r\n");
    rt_kprintf("-------------------------------------------------\n");
    ...
    while((ch_buf = (int)uart_getchar()) != RT_NULL)
    {
        ...
        {
            rt_kprintf("Start the MPU6050 attitude calculation!\n");
            rt_kprintf("\x1b[2J\x1b[H");//执行清屏
            while((ch_buf = (int)uart_getchar()) != RT_NULL)
            {
                mpu6050_RD();
        ...
        else if(ch_buf == 0x33)
        {
            ...
            while((ch_buf = (int)uart_getchar()) != RT_NULL)
            {
                rt_kprintf("The system reports data to the server!\n");
                while(1)
                {
                    cat_dhtxx();
